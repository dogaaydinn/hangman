...
var allWords = countryList
    .Concat(animalList)
    .ToList();

var initialAlphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
var initialPlayerMaxGuess = 6;
...
void GuessWord(string category)
{
    var random = new Random();

    var wordList = category switch
    {
        Category.Animal => animalList,
        Category.Country => countryList,
        _ => allWords
    };
    ...
    {
        var playerMaxGuess = initialPlayerMaxGuess;
        var currentAlphabet = initialAlphabet;
        var chosenWord = wordList[random.Next(wordList.Count)].ToUpper();
...
static string UpdateGuessedWord(string chosenWord, string guessedWord, char chosenLetter)
{
